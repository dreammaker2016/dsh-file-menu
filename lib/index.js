 ...
 * 取出并解析目标路径，解析不出来返回空串。
 *
 * DSH 对工作区内的文件显示的是**相对路径**（工具行 fileLink 按钮更是只有文本、
 * 没有 title），所以客户端上报的可能是相对路径，必须能解析：
 *   1. 绝对路径：直接用（归一化）
 *   2. `~` / `~/...`：展开到家目录
 *   3. 相对路径 + base（客户端上报的当前会话工作区根，绝对路径）：按 base 拼接
 */
function resolveTarget(body) {
  if (body === null || typeof body !== 'object' || typeof body.path !== 'string') return ''
  let value = body.path.trim()
  ...
  if (base !== '' && isAbsolute(base)) return resolve(base, value)
  return ''
}
...
/**
 * 在系统文件管理器中定位路径。
 *   win32  资源管理器 /select 选中该文件（目录则在父目录中选中）
 *   darwin Finder `open -R` 选中
 *   linux  xdg-open 没有「选中」语义，退而打开所在目录
 *
 * ⚠️ Windows 上 `windowsVerbatimArguments: true` 是**必须**的，别改成默认形式：
 * Node 默认会对参数做加引号 + 转义，`/select,"<path>"` 会被拼成 `"/select,\"<path>\""`，
 * Explorer 的非标准命令行解析会忽略这个开关并退回默认文件夹（实测落到「文档」）；
 * 含空格的路径即使把开关与路径拆成两个参数，也只打开目录、不选中文件。
 * 实测三种路径下 verbatim 形式都能正确选中，这也是本插件相对 DSH 自带实现的绕坑点
 * （DSH 用 `['/select,', <file URL>]`，在本机实测同样落到错误目录）。
 */
async function revealInFileManager(target, isDirectory) {
  if (process.platform === 'win32') {
    await spawnDetached('explorer.exe', [`/select,${target}`], { windowsVerbatimArguments: true })
    return
  }
    ...
    () =>
      ctx.webServer.register({
        ...
        handler: async (req, res) => {
          ...
          if (target === '') {
            sendJson(res, 400, { ok: false, error: '无法解析路径（相对路径需要 base）' })
            return
          }
          if (!existsSync(target)) {
            sendJson(res, 404, { ok: false, error: '路径不存在', path: target })
            return
          }
          try {
            if (action === 'reveal') {

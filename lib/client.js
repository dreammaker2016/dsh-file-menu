window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function pathFromTarget(target) {
			...
			while (node !== null && node !== undefined && node.nodeType === 1 && depth < 24) {
				...
				for (var index = 0; index < attrs.length; index += 1) {
					...
					var raw = node.getAttribute(attrs[index]);
					if (looksAbsolutePath(raw)) return raw.trim();
				}
				if (node.dataset !== undefined && node.dataset !== null) {
					for (var key in node.dataset) {
						if (
							...
							/path/i.test(key) &&
							looksAbsolutePath(node.dataset[key])
						) {
				...
				if (tag === "CODE" || tag === "SPAN" || tag === "A" || tag === "BUTTON") {
					var text = String(node.textContent == null ? "" : node.textContent).trim();
		...
		function callFileAction(action, path) {
			report({ phase: "file-action", action: action, path: path });
			postJson("/file-menu/" + action, { path: path }).then(function (result) {
				if (result.ok) {
					toast(action === "reveal" ? "已在资源管理器中定位" : "已用默认应用打开");
					return;
				}
				if (action === "reveal" && result.status === 404) {
					postJson("/open-in-app/open", { app: "explorer", path: parentDirectory(path) }).then(function (fallback) {
		...
		function buildItems(target, event) {
			...
			var path = pathFromTarget(target);
			var selection = "";
			try {
			...
			if (path === "" && looksAbsolutePath(selection)) path = selection;
			...
			if (path !== "") {
				...
				items.push({
					label: "打开文件位置",
					run: function () {
						callFileAction("reveal", path);
					},
				});
				items.push({
					label: "用默认应用打开",
					run: function () {
						callFileAction("open", path);
					},
				});
			}
